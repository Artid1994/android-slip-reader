...
# 1. อัปเดต app/build.gradle.kts เป็น versionCode 19 (1.0.0-beta1r)
...
# 2. แก้ไข MainActivity.kt เพิ่ม ProgressDialog แสดงผลระหว่างสแกนสลิป
...
class MainActivity : AppCompatActivity() {
    ...
    private fun scanFolderDirectly(folderUri: Uri) {
        showLoading("⏳ กำลังอ่านข้อมูลสลิปในแฟ้ม...")
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            ...
            if (folder != null) {
                ...
                newCount = scanRecursive(folder, recognizer)
            }
    ...
    private fun processSlipImage(imageUri: Uri) {
        showLoading("⏳ กำลังอ่านสลิปโอนเงิน...")
        try {
...
echo "✅ เพิ่ม ProgressDialog แสดงสถานะการโหลดและอ่านสลิปใน 1.0.0-beta1r เรียบร้อย!"
